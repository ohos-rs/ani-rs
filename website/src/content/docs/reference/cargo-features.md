...
| `tokio_rt` | 选择内置 Tokio backend，并保留 `spawn` / `block_on` / `spawn_blocking` / `within_runtime_if_available` / `create_custom_tokio_runtime` |
...
已有平台线程池或自有执行器时，只开启 `async-runtime`，并在 `#[ani(init)]` 里 `register_async_runtime(...)`。`ANI_Constructor` 会关闭注册窗口。生成宏、Task 和 TSFN 不要求 backend 是 Tokio。

`tokio_rt` 与自定义 backend 可以同时存在：生成的 async 函数走选中的 backend，Tokio helper 名字保持 Tokio 语义，不会因为 Cargo feature 合并而改路由。
