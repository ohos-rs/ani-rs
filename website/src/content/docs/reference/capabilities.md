---
title: 支持能力总览
description: ani-rs 当前已支持、已验证和明确保留边界的完整能力矩阵。
---

:::note
这页只回答一个问题：“现在 `ani-rs` 到底支持什么，哪些已经过了 example / Docker / ArkVM / OpenHarmony QEMU 回归，哪些还是明确保留的边界？”
:::
...
- 静态 ArkTS HAP 的签名、安装与 Ability 启动验证
...
- OpenHarmony 7.0.0.32（API 26）QEMU 镜像中 52 / 52 个 example 通过
- QEMU runtime 共执行 393 条断言，结果为 `393 passed / 0 failed`
- 静态 ArkTS HAP 安装与启动链路输出 `[ANI_HAP_PASS]`
