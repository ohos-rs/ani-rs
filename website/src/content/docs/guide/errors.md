...
`PromiseFuture<T, E = PromiseRejection>` 允许通过对象安全的 `RejectionDecoder<E>` 解码为任意领域错误。默认 decoder 对 cause/metadata 图执行 object identity 与 visited-set 检查，并限制最大深度、节点数和 binary 大小；循环 cause 使用引用标记保留，不会无限递归。RuntimeTask 取消同样经过可注册的 cancellation error factory 和 materializer。
