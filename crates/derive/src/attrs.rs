...
/// ANI module definition
#[derive(Debug)]
pub struct AniModuleDef {
    /// Module name
    pub name: String,
    /// Library name
    ...
    /// Namespace list (reserved for future use)
    #[allow(dead_code)]
...
impl Parse for AniModuleDef {
    fn parse(input: ParseStream) -> syn::Result<Self> {
        let mut def = AniModuleDef {
            name: String::new(),
            lib_name: None,
            functions: Vec::new(),
            classes: Vec::new(),
        ...
        while !input.is_empty() {
            ...
            match key.to_string().as_str() {
                ...
                "lib_name" => {
                    let lit: LitStr = input.parse()?;
                    def.lib_name = Some(lit.value());
                }
                "functions" => {
                    let content;
                    syn::bracketed!(content in input);
                    let funcs: Punctuated<Ident, Token![,]> =
                        Punctuated::parse_terminated(&content)?;
                    def.functions = funcs.into_iter().collect();
                }
                "classes" => {
                    let content;
                    syn::bracketed!(content in input);
                    let classes: Punctuated<Ident, Token![,]> =
                        Punctuated::parse_terminated(&content)?;
                    def.classes = classes.into_iter().collect();
                }
                "init" => {
                    let ident: Ident = input.parse()?;
                    def.init = Some(ident);
