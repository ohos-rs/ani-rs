...
//!
//! Set `ANI_MODULE_DESCRIPTOR` to the fully qualified Stage module descriptor.
//! It falls back to `CARGO_PKG_NAME` for standalone libraries.
...
use expand::{
    expand_class_derive, expand_enum_derive, expand_finalize, expand_function, expand_impl,
    expand_init, expand_struct,
};
use parser::{AniAttrs, AniMacroKind, BindgenAttrs, FinalizeAttrs, InitAttrs};
...
fn generate_constructor_code() -> proc_macro2::TokenStream {
    if IS_FIRST_ANI_MACRO.swap(false, Ordering::SeqCst) {
        let name = types::current_module_name();
        ...
        quote::quote! {
            ...
            pub unsafe extern "C" fn ANI_Constructor(
                ...
                unsafe {
                    if vm.is_null() || result.is_null() {
                        return ::ani::sys::ani_status_ANI_INVALID_ARGS;
                    }
                    let api = &*(*vm);
                    ...
                    let status = get_env(vm, ::ani::sys::ANI_VERSION_1, &mut env);
            ...
            /// ANI module destructor - auto-generated by the first #[ani] macro.
            #[unsafe(no_mangle)]
            pub unsafe extern "C" fn ANI_Destructor(
                vm: *mut ::ani::sys::ani_vm,
            ) -> ::ani::sys::ani_status {
                if vm.is_null() {
                    return ::ani::sys::ani_status_ANI_INVALID_ARGS;
                ...
                unsafe {
                    ...
                    let mut env: *mut ::ani::sys::ani_env = std::ptr::null_mut();
                    let Some(get_env) = api.GetEnv else {
                        let _ = ::ani::async_runtime::shutdown_runtime_domain();
                        let _ = ::ani::conversions::close_all_managed_resources();
                        return ::ani::sys::ani_status_ANI_ERROR;
                    };
                    let get_env_status = get_env(
                        vm,
                        ::ani::sys::ANI_VERSION_1,
                        &mut env,
                    );
                    ...
                    if let Err(error) = ::ani::async_runtime::shutdown_runtime_domain() {
                    ...
                    if let Err(error) = ::ani::conversions::close_all_managed_resources() {
                        eprintln!("failed to release managed resources: {error}");
                        if status == ::ani::sys::ani_status_ANI_OK {
                            status = ::ani::sys::ani_status_ANI_ERROR;
                        }
                    }
            ...
            /// Compile-time ANI module descriptor used by generated bindings.
...
pub fn ani(attr: TokenStream, item: TokenStream) -> TokenStream {
    ...
    match attrs.kind {
        ...
        AniMacroKind::Finalize => {
            if let Ok(func) = syn::parse::<ItemFn>(item.clone()) {
                return expand_finalize(FinalizeAttrs, func, prepare).into();
            }
            syn::Error::new_spanned(
                proc_macro2::TokenStream::from(item),
                "#[ani(finalize)] can only be applied to functions",
            )
            .to_compile_error()
            .into()
        }
