...
#[repr(transparent)]
#[derive(Copy, Clone)]
pub struct AStaticFieldID {
    internal: ani_field,
}

// Field IDs are valid across threads (not tied to an ANIEnv)
unsafe impl Send for AStaticFieldID {}
unsafe impl Sync for AStaticFieldID {}

impl AStaticFieldID {
    /// Creates a [`AStaticFieldID`] that wraps the given `raw` [`ani_field`]
    ///
    /// # Safety
    ///
...
impl AsRef<AStaticFieldID> for AStaticFieldID {
    fn as_ref(&self) -> &AStaticFieldID {
        self
    }
}

impl AsMut<AStaticFieldID> for AStaticFieldID {
    fn as_mut(&mut self) -> &mut AStaticFieldID {
        self
    }
}
